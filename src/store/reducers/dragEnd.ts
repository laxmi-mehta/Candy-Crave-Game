...
export const dragEndReducer = (
  state: WritableDraft<{
    ...
    score: number; // Ensure score is included in state
  }>
) => {
  ...
  // Update score based on matches found
  if (
    squareBeingReplacedId &&
    validMove &&
    (isARowOfThree || isARowOfFour || isAColumnOfFour || isAColumnOfThree)
  ) {
    if (isARowOfFour) state.score += 4; // Example score for a row of four
    else if (isARowOfThree) state.score += 3; // Example score for a row of three
    else if (isAColumnOfFour) state.score += 4; // Example score for a column of four
    else if (isAColumnOfThree) state.score += 3; // Example score for a column of three
